export default {
    ...
    methods: {
        toInputPage(){
            ...
            else{location.href = "./inputGuestInfoPage.html"}
        ...
        deledtCart(id){
            axios.delete(`${this.apiInfo.url}/api/${this.apiInfo.path}/cart/${id}`).then((res)=>{ 
                this.getCart();
            }).catch((error) => { console.dir(error) });
        },
        changeCartNum(num,id,productId){
            if(this.isChangeNum === false){
                ...
                axios.put(`${this.apiInfo.url}/api/${this.apiInfo.path}/cart/${id}`, sendCart).then((res)=>{ 
        ...
        getCart(){
            axios.get(`${this.apiInfo.url}/api/${this.apiInfo.path}/cart`).then((res) => {
    ...
    mounted() {
        this.bsOffcanvas = new bootstrap.Offcanvas(this.$refs.rightCart)
    },
    template: `<div class="offcanvas offcanvas-end " data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1"
    ...
    :disabled="cartLength === 0" :class="{buttonDisabledCursor : cartLength === 0}">刪除所有</button></div>
        ...
        <tbody>
            <tr v-for="(item,index) in cartData.carts" :key="item.product.title+index">
                ...
                <td><button type="button" class="btn btn-outline-dark" @click="deledtCart(item.id)">刪除</button></td>
            </tr>
        </tbody>
    </table>
